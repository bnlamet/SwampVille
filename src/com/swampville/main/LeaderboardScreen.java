...
public class LeaderboardScreen implements ActionListener {
	...
	public void readCurrentTopScoresFromAppMemory() {
		...
		try {
			File file = new File("src/tmp/scores.ser");
			if (file.length() > 0) {
				FileInputStream fileIn = new FileInputStream("src/tmp/scores.ser");
	...
	public void saveCurrentScores() {
		int[] e = currentTopScores;
		try {
			FileOutputStream fileOut = new FileOutputStream("src/tmp/scores.ser");
